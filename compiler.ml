...
let make_prologue consts_tbl fvars_tbl =
  ...
  let get_const_address const = "const_tbl+"^(string_of_int (calc_off const consts_tbl)) in
  let get_fvar_address const = "fvar_tbl+"^(string_of_int (get_index const fvars_tbl)) in 
    
  let make_primitive_closure (prim, label) =
    "    MAKE_CLOSURE(rax, SOB_NIL, " ^ label  ^ ")
        mov [" ^ (get_fvar_address prim)  ^ "*8], rax" in
